...
# - View name 1
# - View name 2
# View_name_1_last_processed_id: 20175357
...
def lookup_field_ids()
  ...
  endpoint  = "#{@config["JIRA_URL"]}/rest/api/3/project"
  ...
  endpoint  = "#{@config["JIRA_URL"]}/rest/api/3/issuetype"
  response = HTTParty.get(endpoint,
                            :headers    => { "Content-Type" => "application/json" },
                            :basic_auth => @jira_auth,
                            :verify     => true)
  ...
  endpoint  = "#{@config["JIRA_URL"]}/rest/api/3/priority"
  response = HTTParty.get(endpoint,
                            :headers    => { "Content-Type" => "application/json" },
                            :basic_auth => @jira_auth,
                            :verify     => true)
...
def gather_event_actions()
  ...
  event_actions  = HTTParty.get("#{@website}/api/v2/event_actions.json?page=1&per_page=1000", :headers => @headers)
...
def check_for_existing_ticket(view, node)
  ...
  # Build title
  title = @event_actions[view]["variables"]["title"].gsub(/{{\s*node\s*}}/, node)
...
def add_attachment(issue_key, file_contents)
  ...
  file_path = "tmp/#{issue_key}_file_events_#{DateTime.now.strftime("%Y-%m-%d %H:%M:%S")}.csv"
...
def create_new_ticket(view, node, file_contents)
  ...
  title         = action["variables"]["title"].gsub(/{{\s*node\s*}}/, node)
...
def process_events()
  @event_data.each_pair do |view, view_data|
    ...
    view_data.each_pair do |node, data|
      ...
      matching_issue = check_for_existing_ticket(view, node)
