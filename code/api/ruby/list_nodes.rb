...
Optional flags:
  -disableSSL: Disable SSL certificate verification
  --status: filters results by node status
  --lastScanStatus: filter results by nodes' last scan status
Usage Examples:
...
#Flag Parsing
options = {}
options[:SSLCert] = true
options[:status] = nil
options[:lastScanStatus] = nil

opt_parser = OptionParser.new do |opt|
  opt.on('-disableSSLCert') do
    options[:SSLCert] = false
  end
  ...
  opt.on('--lastScanStatus=LASTSCANSTATUS') do |l|
    options[:lastScanStatus] = l
  end
end
...
if (options[:lastScanStatus] != nil)
  query += "last_scan_status=#{options[:lastScanStatus]}&"
end
...
while !response.nil? and response.count == per_page
  ...
  response = HTTParty.get(link, headers: header, :verify => options[:SSLCert])
