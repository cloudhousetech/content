  ...
  def process
    ...
    if status == 'changed'
      ...
      if lookup["node_id"]
        ...
        Puppet.err("Failed to lookup node: #{lookup}")
      end

      job = node_scan(API_KEY, APPLIANCE_URL, node_id, manifest_filename)

      if job["job_id"]
        Puppet.info("Node scan kicked off against #{node_ip_hostname} (#{APPLIANCE_URL}/jobs/#{job["job_id"]}/show_job?show_all=true)")
      else
        Puppet.err("Failed to kick off node scan against #{node_ip_hostname} (#{node_id}): #{job}")
      end
    end
  end
end
...
def node_create(api_key, instance, ip_hostname, windows_cm_group_id)
  ...
  if node["id"]
    ...
    Puppet.err("Failed to create node: #{ip_hostname}")
  end
end
